...
const LabelPreview = forwardRef<HTMLDivElement, LabelPreviewProps>((props, ref) => {
  const { text, logoColor, pattern, patternColor, className, forceScale } = props;
  ...
  const getOptimalGlowColor = useMemo(() => {
    ...
    // If the average luminance is high (light colors), use dark shadow
    // If the average luminance is low (dark colors), use light glow
    if (avgLuminance > 0.4) {
      // Light background - use lighter dark shadow for subtle contrast
      return "rgba(0, 0, 0, 0.2)";
    } else {
      // Dark background - use light glow (perfect as is!)
  ...
  const handleMouseMove = (e: React.MouseEvent) => {
    ...
    if (isResizing && containerRef.current && onTextSizeChange) {
      ...
      const newWidth = Math.max(50, textSize.width + deltaX);
      const newHeight = Math.max(30, textSize.height + deltaY);
    ...
    >
      <div className="relative w-full h-full flex items-center justify-center">
        ...
        <div 
          ...
          style={{
            filter: `drop-shadow(0 0 15px ${getOptimalGlowColor.replace('0.8', '0.8')}) drop-shadow(0 0 30px ${getOptimalGlowColor.replace('0.8', '0.6')}) drop-shadow(0 0 45px ${getOptimalGlowColor.replace('0.8', '0.4')})`
          }}
        >
        ...
        >
          <div 
            ...
            style={{ 
              color: pattern === "none" ? "#000000" : patternColor,
